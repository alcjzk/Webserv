...
int TiniTree::contextSwitch(std::string ctx, int row) {
  ...
  for (size_t i = 0; i < map_ops.size(); ++i) {
    if (_current->getType() == TiniNode::T_MAP ||
        _current->getType() == TiniNode::T_VECTOR) {
      if (i == map_ops.size() - 1) {
        tmp = &_current->fetchTiniNode(map_ops[i]);
        if (tmp == _current) {
          if (vec) {
            if (_current->getType() == TiniNode::T_VECTOR)
              _current->getVectorValue().push_back(
                  new TiniNode(TiniNode::T_VECTOR));
            else
              _current->getMapValue()[map_ops[i]] =
                  new TiniNode(TiniNode::T_VECTOR);
          } else {
            if (_current->getType() == TiniNode::T_VECTOR)
              _current->getVectorValue().push_back(
                  new TiniNode(TiniNode::T_MAP));
            else
              _current->getMapValue()[map_ops[i]] =
                  new TiniNode(TiniNode::T_MAP);
          }
        ...
        if (vec) {
          if (tmp->getType() != TiniNode::T_VECTOR)
            throw std::runtime_error(
                "TiniTree: contextSwitch: Expected vector value");
      ...
      throw std::runtime_error(
          "TiniTree: contextSwitch: Unexpected lone string in tree");
  }
  return (0);
}
...
int TiniTree::valueInsertion(std::string act, int row) {
  ...
  case TiniNode::T_VECTOR:
    throw std::runtime_error(
        "TiniTree: valueInsertion: Vector cannot act as endpoint");
    break;
  case TiniNode::T_MAP:
  ...
  case TiniNode::T_STRING:
    throw std::runtime_error(
        "TiniTree: valueInsertion: String cannot act as endpoint");
    break;
  default:
